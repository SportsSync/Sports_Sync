...
// Fetch vendor turf status
$sql = "SELECT status FROM turftb WHERE owner_id=?";
...
// No turf registered
if (!$turf) {
    echo "<h2 style='color:white'>You have not registered a turf yet.</h2>";
    exit;
}
...
if ($turf['status'] !== 'approved') {
    echo "
    <h2 style='color:white'>Vendor request not approved</h2>
    <p>Status: <b>{$turf['status']}</b></p>
